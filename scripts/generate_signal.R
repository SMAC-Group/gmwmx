...
# define model for generating gaussian white noise + RW
...
# define A matrix
A <- create_A_matrix(1:n, jump_vec, n_seasonal = 2)
...
# plot empirical distribution of estimated stochastic parameters
boxplot(mat_res[, c(8)])
abline(h = sigma2_wn)
boxplot(mat_res[, c(9)])
abline(h =gamma2_rw)
